...
def write_activation_example(decode, tokens, activations, logits_diff):
    ...
    for j in range(W):
        ...
        logit_diff = logits_diff[j].item()
        # TODO: make sure my writing of logit_diff is not off by one token in position
        ...
        underline_color = "underline-red" if logit_diff > 0 else "underline-blue" if logit_diff < 0 else ""
        single_token_text = f"""
        <div class="tooltip">
            <span class="{text_color} {underline_color}"> {start_bold_text(j, mid_token_index)} {char.replace(' ', '&nbsp;')} {end_bold_text(j, mid_token_index)} </span>
            <span class="tooltiptext">
                Token: {char}
                Activation: {activation:.4f}
                Feature Ablation: {logit_diff:.4f}
            </span>
        </div>"""
...
# TODO: merge write_alive_feature_page and write_ultralow_density_feature_page into one single function
...
def write_alive_feature_page(feature_id, decode, top_logits, bottom_logits, top_acts_data, sampled_acts_data, dirpath=None):
    ...
    assert (
        'tokens' in top_acts_data.keys()
        and 'feature_acts' in top_acts_data.keys()
        and 'tokens' in sampled_acts_data.keys()
...
def write_ultralow_density_feature_page(feature_id, decode, top_acts_data, dirpath=None):
    ...
    assert (
        'tokens' in top_acts_data.keys()
        and 'feature_acts' in top_acts_data.keys()
        and "expect input TensorDict to have tokens and features_acts keys"
    )
